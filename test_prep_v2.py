## test_preparation
# ver1 
#   prepare lpc file from wav files of IRMAS dataset (test)
#   slice data
...
lpc_folder = test_data_directory + 'lpcs_processed/' #lpc path
...
for orig_testdata_path in orig_data_directory_testlist: #part1,2,3
    ...
    #since each music file has difefrent length, generate lpc files for each music
    ...
    for i, musicname in enumerate(musicname_list):
            ...
            with open(txtfile_path, 'r') as file:
                ...
                for j, instID in enumerate(label_list):
                    for k in range(numslice):
                        ...
                        lpc = generate_lpc_v1.main(audio_sliced, lpc_order, fs)  #audio feature extraction method
        ...
        lpc_filename = lpc_folder + 'lpcs_' + musicname + '_n' + str(lpc_order) + '.npy'
